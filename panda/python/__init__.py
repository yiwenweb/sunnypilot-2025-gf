...
class Panda:
  ...
  def connect(self, claim=True, wait=False):
    ...
    # A) oldest bootstubs don't have any way to distinguish MCU or panda type
    # B) slightly newer bootstubs have the panda type set in the USB bcdDevice
  ...
  def get_type(self):
    ret = self._handle.controlRead(Panda.REQUEST_IN, 0xc1, 0, 0, 0x40)
    if self.bootstub and ret.startswith(b'\xff\x00\xc1\x3e\xde\xad\xd0\x0d') and self._bcd_hw_type is not None:
      return self._bcd_hw_type
    return ret
  ...
  def get_mcu_type(self) -> McuType:
    ...
    if hw_type in Panda.F4_DEVICES:
      ...
      return McuType.H7
    elif self.bootstub and getattr(self, '_assume_f4_mcu', False):
      return McuType.F4
