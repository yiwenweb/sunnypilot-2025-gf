...
from openpilot.sunnypilot.models.runners.constants import CLMemDict, FrameDict, NumpyDict, ModelType, ShapeDict, SliceDict
from openpilot.sunnypilot.models.runners.model_runner import ModelRunner, get_model_file_path
...
class TinygradRunner(ModelRunner, SupercomboTinygrad, PolicyTinygrad, VisionTinygrad, OffPolicyTinygrad, OnPolicyTinygrad):
  ...
  def __init__(self, model_type: int = ModelType.supercombo):
    ...
    # 使用 get_model_file_path 优先从 customer 目录加载
    model_pkl_path = get_model_file_path(artifact_filename)
...
class TinygradSplitRunner(ModelRunner):
  """
  A ModelRunner that coordinates separate vision, policy, offPolicy and onPolicy runners.

  Manages the execution of split models, combining their inputs and outputs.
  Supports 2-way (vision+policy), 3-way (vision+offPolicy+onPolicy) and mixed splits.
  """
  def __init__(self):
    super().__init__()
  ...
  def _run_model(self) -> NumpyDict:
    """Runs vision and all policy runners, merging their parsed outputs."""
  ...
  def input_shapes(self) -> ShapeDict:
    """Returns the combined input shapes from all runners."""
  ...
  def output_slices(self) -> SliceDict:
    """Returns the combined output slices from all runners."""
  ...
  def prepare_inputs(self, imgs_cl: CLMemDict, numpy_inputs: NumpyDict, frames: FrameDict) -> dict:
    """Prepares inputs for all runners."""
    if self.policy_runner:
